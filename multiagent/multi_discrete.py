...
# XXX: is this enhancement necessary ?
class MultiDiscrete(multi_discrete.MultiDiscrete):
    ...
    @property
    def n(self):
        return reduce(mul, self.nvec.shape)
...
#
#         # TODO(ming): flatten
#         self.w = self.high[0] - self.low[0]
#         self.h = self.high[1] - self.low[1]
#
#         self.num_discrete_space = self.w * self.h
#
#     def sample(self):
...
#         return prng.np_random.rand(self.num_discrete_space)
#
#     def contains(self, x):
#         row, col = x // self.w, x % self.w
#
#         return row <= self.h - 1 and col <= self.w - 1
